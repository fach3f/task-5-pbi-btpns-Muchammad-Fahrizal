...
func RegisterUser(c *gin.Context) {
	...
	if result := database.DB.Create(&user); result.Error != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Email Has Been Registered, Try to use another email",
		})
		return
	}
...
func LoginUser(c *gin.Context) {
	...
	// Membuat token
	tokenStr, err := helpers.InitializeToken(user.ID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": err.Error(),
		})
...
func UpdateUser(c *gin.Context) {
	...
	if input_user.Password != "" {
		hashedPass := helpers.EncryptPassword(c, input_user.Password)
		if hashedPass == "" {
			return
		}
